...
train_loc = np.setdiff1d(np.asarray(range(len(currentdata_x))), test_loc)
# seperate all the data into train, test and validation set with the propotion of 8:1:1
# train_loc = np.asarray(sorted(random.sample(range(len(currentdata_x)), round((1-nn_split_ratio)*len(currentdata_x))))) 
...
########################################################
# fit network
history = model.fit(x = train_x, y = train_y, epochs = nn_epochs, batch_size = nn_batch_size, validation_split = nn_split_ratio, verbose = 2)
# fig = plt.figure()    
...
for ipara in range(len(para_names)):
	...
	plt.plot(test_y[:, ipara], nn_predict[:, ipara], ',', markersize = 0.05, markeredgecolor = 'red', markerfacecolor = 'red')
	# plt.hexbin(test_y[:, ipara], nn_predict[:, ipara], gridsize=30, cmap='Blues')
	# cb = plt.colorbar(label='count in bin')
	plt.plot(linear_x, linear_y, '-k', linewidth = 1)
	plt.ylim(0, 1)
	plt.xlim(0, 1)
