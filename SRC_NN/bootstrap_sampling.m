...
% profile lon and lat info
profile_lon_lat = load([data_dir_input, 'wosis_2019_snap_shot/wosis_2019_snapshot_hugelius_mishra_env_info.mat']);
profile_lon_lat = profile_lon_lat.EnvInfo(profile_env_info(:, 1), [4, 5]);


%% traditional sampling
site_loc = 1:length(profile_env_info(:, 1));

candidate_site_loc = site_loc;

train_loc = nan(length(site_loc), bootstrap_num);
for isample = 1:bootstrap_num
    train_loc(:, isample) = sort(datasample(candidate_site_loc, length(site_loc), 'Replace', true));

end

save([data_dir_input, 'data4nn/bootstrap_train_loc_', model_name, '_', time_domain, '.mat'], 'train_loc');
