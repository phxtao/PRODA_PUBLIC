...
global_simu = readMat(paste(data_dir_output, 'world_simulation_analyses/marginal_sensitivity_proda_NPP_', model_name, '_', nn_exp_name, '_cross_valid_0_', as.character(7), '.mat', sep = ''))

bulk_process_A = Re(global_simu$var.data.middle[[4]][[1]])[ , 11]
bulk_process_K = Re(global_simu$var.data.middle[[5]][[1]])[ , 11]
bulk_process_Xi = Re(global_simu$var.data.middle[[6]][[1]])[ , 11]
bulk_process_V = Re(global_simu$var.data.middle[[7]][[1]])[ , 11]
bulk_process_I = Re(global_simu$var.data.middle[[8]][[1]])[ , 11]
bulk_process_NPP = Re(global_simu$var.data.middle[[9]][[1]])[ , 11]

global_simu_process = cbind(bulk_process_A, bulk_process_I, bulk_process_K, bulk_process_V, bulk_process_Xi, bulk_process_NPP)
global_simu_soc = cbind(Re(global_simu$var.data.middle[[2]][[1]])[ , 11], Re(global_simu$var.data.middle[[3]][[1]])[ , 11])
...
jpeg(paste('./Ensemble/revision1_main_fig3.jpeg', sep = ''), width = 35, height = 25, units = 'in', res = 300)
