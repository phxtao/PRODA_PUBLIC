...
####################################################################
# CUE-SOC standarized value
####################################################################
current_data$cue_standard = NA
current_data$depth_standard = NA
current_data$mat_standard = NA

isource = 1
for (isource in 1:17) {
  source_loc = which(current_data$source_id == isource)
  #cue
  if (is.na(sd(current_data$cue[source_loc], na.rm = TRUE)) == 1 || 
      sd(current_data$cue[source_loc], na.rm = TRUE) == 0) {
    current_data$cue_standard[source_loc] = 0
  } else {
    current_data$cue_standard[source_loc] = (current_data$cue[source_loc] - mean(current_data$cue[source_loc], na.rm = TRUE))/sd(current_data$cue[source_loc], na.rm = TRUE)
  }
  
  #depth
  if (is.na(sd(current_data$depth[source_loc], na.rm = TRUE)) == 1 || 
      sd(current_data$depth[source_loc], na.rm = TRUE) == 0) {
    current_data$depth_standard[source_loc] = 0
  } else {
    current_data$depth_standard[source_loc] = (current_data$depth[source_loc] - mean(current_data$depth[source_loc], na.rm = TRUE))/sd(current_data$depth[source_loc], na.rm = TRUE)
  }
  
  #mat 
  if (is.na(sd(current_data$mat[source_loc], na.rm = TRUE)) == 1 || 
      sd(current_data$mat[source_loc], na.rm = TRUE) == 0) {
    current_data$mat_standard[source_loc] = 0
  } else {
    current_data$mat_standard[source_loc] = (current_data$mat[source_loc] - mean(current_data$mat[source_loc], na.rm = TRUE))/sd(current_data$mat[source_loc], na.rm = TRUE)
  }
...
# 
# ggplot(data = current_data) + 
#   geom_point(aes(x = cue, y = soc, group = source_id, color = source_id)) + 
#   geom_smooth(aes(x = cue, y = soc, group = source_id, color = source_id), fill = NA,  method = 'lm')

## mixed effects model
mix_model = lmer(log10(soc) ~ cue_standard + depth_standard + mat_standard + (1 | source_id), data = current_data)
...
fix_effect_model = summary(mix_model)
fix_effect_pred = fix_effect_model$coefficients[1, 1] + 
  fix_effect_model$coefficients[2, 1]*current_data$cue_standard + 
  fix_effect_model$coefficients[3, 1]*current_data$depth_standard + 
  fix_effect_model$coefficients[4, 1]*current_data$mat_standard
r_squared_fixed = summary(lm(log10(current_data$soc) ~ fix_effect_pred))
r_squared_fixed
