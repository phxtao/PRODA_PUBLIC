function [cpool_current, soc_stock, soc_layer] = matrix_fun_model_analysis(kp, nbedrock, sand_vector, npp_mean, ...
...
bio = 10^(-4); % kp(1)*(5*10^(-4) - 3*10^(-5)) + 3*10^(-5);
% cryoturbation 5*10^(-4) (m2/yr)
cryo = 5*10^(-4); % kp(2)*(16*10^(-4) - 3*10^(-5)) + 3*10^(-5);
% ------------env. scalar
% Q10 (unitless) 1.5
q10 = 1.5; % kp(3)*(3 - 1.2) + 1.2;
...
beta = 0.95; % kp(22)*(0.9999 - 0.5) + 0.5;
% allometric slope for microbial vs enzyme production relationship
allo_slope_mic = kp(1);%-0.05;
...
cpool_initial = 1*ones(npool_vr, 1); % zeros(npool_vr, 1);
...
disp([datestr(now), ' simulation finished'])
